//  V.5.2c JA9OIR Version (Use ESP32-2432S028R)
...
//#include <TFT_eSPI.h>
#define LGFX_USE_V1         // V.5.2c
#include <LovyanGFX.hpp>    // V.5.2c
#include "LovyanGFX_userconfig28.h"
#include <SPI.h>
#include <SI4735.h>
#include "EEPROM.h"
#include "Rotary.h"
...
//   Band table
Band band[] = {
  {   "FM", FM_BAND_TYPE,  FM,  7600, 10800,  8470, 10, 0, 0}, //  FM          0
  {   "LW", LW_BAND_TYPE,  AM,   100,   514,   198,  9, 0, 0}, //  LW          1
  {   "MW", MW_BAND_TYPE,  AM,   514,  1800,   810,  9, 0, 0}, //  MW          2
  ...
  {  "80M", SW_BAND_TYPE, LSB,  3500,  3900,  3530,  1, 0, 0}, // Ham   80M    8
  ...
  {  "40M", SW_BAND_TYPE, LSB,  7000,  7500,  7050,  1, 0, 0}, // Ham   40M   12
  ...
  {  "20M", SW_BAND_TYPE, USB, 14000, 14500, 14150,  1, 0, 0}, // Ham   20M   18
  ...
  {  "14M", SW_BAND_TYPE, USB, 21000, 21500, 21250,  1, 0, 0}, // Ham   14M   23
...
LGFX tft;
lgfx::LGFX_Sprite spr(&tft);      // ex [64 * 64] Buffer
...
//  Wire.begin(ESP32_I2C_SDA, ESP32_I2C_SCL); //I2C for SI4735
 ...
 //Serial.println("start message");while(1);
...
void drawBut(uint8_t id, uint8_t state, String alt) {
  ...
  if (state == B_JAM) spr.setTextColor(TFT_GREENYELLOW);
  else
  if (state == B_SELECT) spr.setTextColor(TFT_GOLD);
  else
  if (type == B_GOLD) spr.setTextColor(TFT_WHITE);
  else spr.setTextColor(TFT_BLUE);
  spr.drawString(alt, (But_Width / 2) + 3, (But_Height / 2) + 10);
  if (state != B_JAM && state != B_SELECT) {
    if (state == B_BLOCK || type != B_GOLD) spr.setTextColor(TFT_YELLOW); else spr.setTextColor(TFT_OLIVE);
